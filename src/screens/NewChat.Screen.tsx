...
const NewChatScreen = (props: Props) => {
    ...
    const [selectedUserIds, setSelectedUserIds] = useState<Array<keyof Users>>(
        []
    ); // array of user ids for group chat
    ...
    const isGroupChatDisabled =
        selectedUserIds.length === 0 || (chatName === "" && isNewGroupChat);
    ...
    const handleUserPressed = (otherUserId: string) => {
        if (isGroupChat) {
            ...
            dispatch(
                setStoredUsers({
                    newUsers: { [otherUserId]: users[otherUserId] },
                })
            );
        } else {
            // chat with this user already exists
            let chatData = userChats.find(
                (chat) => !chat.isGroupChat && chat.users.includes(otherUserId)
            );
