...
export const createChat = async (
    ...
    try {
        ...
        for (let i = 0; i < chatUsers.length; i++) {
            ...
            //         ...
            //     },
            const newUserChat = await push(
                child(dbRef, `userChats/${userId}`),
                newChat.key
            );
        }
        ...
        throw new Error(
            "Something went wrong while creating a new chat. Please try again later."
        );
    }
};
...
export const sendInfoMessage = async (
    data: Omit<SendMessageParams, "type" | "imageUrl" | "replyTo">
) => {
    await sendMessage({
        ...data,
        type: "info",
...
export const editChatMessage = async (data: {
    chatId: string;
    messageId: string;
    text: string;
    userId: string;
...
export const markMessageAsSeen = async (data: MarkMessageAsSeenParams) => {
    try {
        ...
        const messageSeenRef = child(
            dbRef,
            `messages/${chatId}/${messageId}/seen`
        );
